A java native application that implements the below betting algorithm!

To build the program first make a mvn clean install 
To run, java -jar target/scratch-game-1.0-SNAPSHOT.jar --config src/main/resources/config.json --betting-amount 100
Note that the "src/main/resources/config.json" file can be at any path
...
from symbols(based on probabilities for each individual cell) and based on winning combintations user either will win or lost.
...
**Standard Symbols**: identifies if user won or lost the game based on winning combinations (combination can be X times repeated 
symbols or symbols that follow a specific pattern)
...
Calculations: (bet_amount x reward(symbol_A) x reward(same_symbol_5_times) x reward(same_symbols_vertically))
 + (bet_amount x reward(symbol_B) 
 x reward(same_symbol_3_times) 
 x reward(same_symbols_vertically)) 
 (+/x) reward(+1000) = (100 x5 x5 x2) + (100 x3 x1 x2) + 1000 = 5000 + 600 + 1000 = 6600
...
```bash
 jar xf target/scratch-game-1.0-SNAPSHOT.jar META-INF/MANIFEST.MF
